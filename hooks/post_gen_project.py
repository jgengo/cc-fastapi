...
def run_cmd(cmd: list[str], description: str) -> bool:
    try:
        ...
        print(f"{SUCCESS} {description}{EOL}")
        return True
    except (FileNotFoundError, subprocess.CalledProcessError):
        print(f"{ERROR} {description} failed{EOL}")
        return False


print(f"{SUCCESS} Sucesfully created project{EOL}")

shutil.copy("env.sample", ".env")
print(f"{SUCCESS} Copied env.sample to .env{EOL}")
...
print(f"{INFO} Commiting files, might take a moment because of the pre-commit hooks...{EOL}")
run_cmd(["git", "commit", "-m", "Initial commit"], "Committed files")
print(f"{SUCCESS} Succesfully created project, now you can start developing!{EOL}")